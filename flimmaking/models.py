...
class flimSection(models.Model):
...
class flimFeature(models.Model):
...
class flimCoursesyllbi(models.Model):
...
class flimCourseSyllabus(models.Model):
    course = models.ForeignKey(flimCoursesyllbi, related_name='syllabus', on_delete=models.CASCADE)
    title = models.CharField(max_length=255)
    description = models.TextField()
...
class flimDuration(models.Model):
    course = models.OneToOneField(flimcourse, on_delete=models.CASCADE)
...
class flimCardSliderItem(models.Model):
